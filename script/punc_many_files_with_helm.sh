...
for file in $files; do
  ...
  i=$((i+1))
  cat $file >> all_asr_out
done

echo "Step 2: Punctuating with very big HELM"
hidden-ngram -text all_asr_out -order 5 -vocab ~/data/punct/vocab -hidden-vocab ~/data/online_punc_ngram/hidden-vocab -keep-unk -lm $LM > all_hyp_helm
...
for j in `seq 0 $((i-1))`; do
  ...
  head -$processed all_hyp_helm | tail -${lines[j]} > $OUT_DIR/${names[j]}
done
