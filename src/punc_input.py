...
def inputs(data_dir, num_steps=20, batch_size=1, tfrecords_format="tfrecords-*", mode="words"):
    ...
    features = tf.parse_single_example(
        ...
        features={
            ...
            "masks": tf.FixedLenFeature([EXAMPLES_PER_FILE], tf.int64),
        })
    ...
    label_data = tf.transpose(
                    tf.reshape(
                        ...
                        [batch_size, batch_len]))
    mask_data = tf.transpose(
                    tf.reshape(
                        tf.slice(mask, [0], [batch_size * batch_len]), 
                        [batch_size, batch_len]))

    num_threads = 32
    capacity = 20000 + 20 * batch_size

    input_batch, label_batch, mask_batch = tf.train.batch(
        [input_data, label_data, mask_data], batch_size=num_steps, num_threads=num_threads, 
        capacity=capacity, enqueue_many=True, shapes=[[batch_size], [batch_size], [batch_size]])

    input_batch = tf.transpose(input_batch)
    label_batch = tf.transpose(label_batch)
    mask_batch = tf.transpose(mask_batch)
